...
df = spark.read.json('s3://geron-raw/nov-22/'+'*.json')
...
df_completa = df_completa.withColumn('year', F.year('tweet_date')) \
            .withColumn('month', F.lpad(F.month('tweet_date'), 2, '0')) \
            .withColumn('day', F.dayofmonth('tweet_date'))

df_completa.write.parquet("s3://geron-ref/Desafio_XPTO_Semana10/", mode = 'overwrite')
